...
export class BptreeNode {
	...
	static nodeBg = "#00ba38";
	static cellBg = "#7ddb99";
	static fontSize = 12;
	...
	constructor(M: number, isLeaf: boolean, dontAlloc?: boolean) {
		...
		this.totalWidth = (M - 1) * BptreeNode.cellWidth;
	...
	resetStyle() {
		...
		this.keysBg = new Array(this.M - 1).fill(BptreeNode.cellBg);
	}

	drawBorder(ctx: CanvasRenderingContext2D, color: string) {
		ctx.strokeStyle = color;
		ctx.lineWidth = BptreeNode.borderWidth;
		const wb2 = ctx.lineWidth / 2;
		ctx.beginPath();
		ctx.roundRect(this.x - wb2, this.y - wb2, this.totalWidth + ctx.lineWidth, BptreeNode.cellHeight + ctx.lineWidth, 4);
		ctx.stroke();
		ctx.restore();
	}
	...
	drawBg(ctx: CanvasRenderingContext2D) {
		...
		ctx.fillRect(x, y, this.totalWidth, BptreeNode.cellHeight);
	...
	drawCell(ctx: CanvasRenderingContext2D, idx: number) {
		...
		const curX = (BptreeNode.cellWidth * idx) + padb2;

		ctx.beginPath();
		ctx.fillStyle = this.keysBg[idx];
		ctx.roundRect(x + curX, y + padb2, BptreeNode.cellWidth - pad, BptreeNode.cellHeight - pad, 4);
		ctx.fill();
		ctx.restore();
		...
		ctx.font = `${BptreeNode.fontSize}px monospace`;

		let text = String(this.keys.v.arr[idx]);
		
		ctx.fillText(text, x + curX + (BptreeNode.cellWidth / 2) - pad, (this.top + this.bottom) / 2);
	}
	...
	get bottom() {
		return this.y + BptreeNode.cellHeight;
	}
	...
	intersects(x: number, y: number, transform: Transform): boolean {
		...
		const highy = lowy + (BptreeNode.cellHeight * transform.scale);
