...
class DeleteLLRbtree extends AlgorithmHandler {
	...
	*merge(node: ElementLLRbtreeNode, idx: number, canvas: CanvasHandler) {
		if(Null.isNull(node.children.v.arr[idx]) || Null.isNull(node.children.v.arr[idx + 1])) {
			return;
		}

		let child = (node.children.v.arr[idx] as Ptr<ElementLLRbtreeNode>).v;
		let sibbling = (node.children.v.arr[idx + 1] as Ptr<ElementLLRbtreeNode>).v;
		...
		yield;
		child.bg = LLRbtreeNode.nodeBg;
		child.draw(canvas.ctx);
		sibbling.bg = LLRbtreeNode.nodeBg;
		sibbling.draw(canvas.ctx);
	...
	*borrowFromPrev(node: ElementLLRbtreeNode, idx: number, canvas: CanvasHandler) {
		if(Null.isNull(node.children.v.arr[idx]) || Null.isNull(node.children.v.arr[idx - 1])) {
			return;
		}
	...
	*borrowFromNext(node: ElementLLRbtreeNode, idx: number, canvas: CanvasHandler) {
		if(Null.isNull(node.children.v.arr[idx]) || Null.isNull(node.children.v.arr[idx + 1])) {
			return;
		}
	...
	*fill(node: ElementLLRbtreeNode, idx: number, canvas: CanvasHandler) {
		const T = node.T;
		const prevChild = (node.children.v.arr[idx - 1] as Ptr<ElementLLRbtreeNode>);
		const nextChild = (node.children.v.arr[idx + 1] as Ptr<ElementLLRbtreeNode>);
	...
	*deleteFromLeaf(node: ElementLLRbtreeNode, idx: number, canvas: CanvasHandler) {
		for(let i = idx + 1; i < node.curKeyCount.value; i++) {
			for(const _ of node.animateCellBg(canvas, i, Color.shifting)) yield;
			node.keys.v.arr[i - 1] = node.keys.v.arr[i];
			for(const _ of node.animateCellBg(canvas, i - 1, Color.shifting)) yield;
	...
	indexOfChild(parent: ElementLLRbtreeNode, child: ElementLLRbtreeNode) {
		for(let i = 0; i <= parent.curKeyCount.value; i++) {
			if((parent.children.v.arr[i] as Ptr<ElementLLRbtreeNode>).v === child) {
				return i;
			}
		}
		return -1;
	...
	*indexOfKey(node: ElementLLRbtreeNode, key: number, canvas: CanvasHandler) {
	...
	*deleteIterative(root: ElementLLRbtreeNode, value: number, canvas: CanvasHandler) {
		const T = root.T;
		let stack: Array<ElementLLRbtreeNode> = [];

		let node: Ptr<ElementLLRbtreeNode> | Null = root.ptr;
		let foundInInternalNode: null | ElementLLRbtreeNode = null;
		...
		if(flag) {
			...
			while(true) {
				...
				if(result.done) {
					...
					if(isChangeLeaf) {
						leaf = (parent.children.v.arr[parent.curKeyCount.value] as Ptr<ElementLLRbtreeNode>).v;
					}
					break;
				}
				yield;
	...
	*deleteValue(root: ElementLLRbtreeNode, value: number, canvas: CanvasHandler) {
		...
		if(root.curKeyCount.value === 0) {
			...
			if(!root.isLeaf.value) {
				...
				if(!Null.isNull(child)) {
					(child as Ptr<ElementLLRbtreeNode>).v.parentNode = null;
					root = (child as Ptr<ElementLLRbtreeNode>).v;
				}
			}
			temp.remove(canvas);
		}
